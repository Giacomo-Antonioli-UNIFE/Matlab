...
pause
%
% Risoluzione del sistema mediante il metodo SOR
%
m = 5;
...
figure
ph = plot(omega, itSOR, '-b', 'LineWidth', 2);
title('Iterazioni necessarie per SOR al variare di omega');
pause
D = spdiags(d0, 0, n, n);
L = -tril(A, -1); U = -triu(A, 1);
for i = 1 : m
G= (D - omega(i)*L) \ ( (1 - omega(i))*D + omega(i)*U );
rho(i) = abs( eigs(G, 1) );
end
figure;
plot(omega, rho, '-r', 'LineWidth', 2);
title('Raggi spettrali di G_{SOR} al variare di omega');
